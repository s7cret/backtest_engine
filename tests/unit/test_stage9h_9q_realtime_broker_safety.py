...
class _Runtime:
    ...
    def update_realtime_tick(self, tick: object) -> Bar:
        ...
        return Bar(
            time=int(getattr(tick, "time") or 0),
            open=10,
            high=price,
            low=10,
            close=price,
...
class _EveryTickOcaCommands(_BaseStrategy):
    def _process_bar(self, bar: Bar, bar_index: int) -> None:
        self.seen.append(bar.close)
        self.ctx.entry(
            f"A{bar_index}", "long", qty=1, oca_name="grp", oca_type="cancel"
        )
        self.ctx.order(
            f"B{bar_index}",
            "short",
...
def _slice() -> BarTickSlice:
    return BarTickSlice(
        ...
        ticks=(
            Tick(110, 10.5, volume=1),
            Tick(120, 11.0, volume=1),
            Tick(150, 9.5, volume=1),
        ),
    )
...
def test_final_tick_rejected_order_restores_preexisting_command_buffer() -> None:
    ...
    assert [(cmd.name, cmd.kwargs["id"]) for cmd in ctx.buffer.commands] == [
        ("entry", "PRE_BUFFER")
    ]
    assert strategy.seen == []


def test_rejected_final_tick_order_leaves_broker_orders_fills_and_trades_unchanged() -> (
    None
):
    engine = _engine()
    _seed_broker(engine)
...
def test_realtime_broker_snapshot_is_deep_copied_for_rejected_final_commit() -> None:
    ...
    assert (
        engine.closed_trades == before_closed
        and engine.closed_trades is not before_closed
    )
...
def test_discarded_every_tick_limit_stop_exits_do_not_leak_commands() -> None:
    ...
    engine._guarded_realtime_strategy_tick_loop_skeleton(
        _slice(), ctx=ctx, strategy=strategy, runtime=_Runtime()
    )
    ...
    with pytest.raises(ConfigError, match="not proven"):
        engine._guarded_realtime_strategy_tick_loop_skeleton(
            ...
            commit_policy=RealtimeTickCommitPolicy(
                ...
                intrabar_order_fill_oracle_proof=RealtimeOrderFillOracleStatus(
                    status="blocked"
                ).as_proof(),
            ),
        )
