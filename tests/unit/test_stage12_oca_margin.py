...
def cfg(**kw):
    d = dict(
        symbol="S", timeframe="1D", start_time=1, end_time=5, commission_type="none"
    )
    d.update(kw)
...
def test_oca_cancel_entry_fills_one_and_cancels_sibling():
    ...
    assert [(t.entry_id, t.qty, t.entry_price) for t in r.open_trades] == [
        ("L_FAST", 1.0, 9)
    ]
    assert any(
        e.code == "ORDER_CANCELLED" and e.order_id == "L_SLOW" for e in (r.events or [])
    )
...
def test_nonstandard_margin_runs_without_unsupported_warning_when_no_call():
    ...
    r = BacktestEngine(cfg(margin_long=50, unsupported_margin_policy="error")).run(
        BuyOnce, bars=bars
    )
    assert r.status == "completed"
    assert not any(d.code == "UNSUPPORTED_MARGIN_LIQUIDATION_MODEL" for d in r.warnings)

    r = BacktestEngine(cfg(margin_short=50, unsupported_margin_policy="warn")).run(
        BuyOnce, bars=bars
    )
    assert r.status == "completed"
    assert not any(d.code == "UNSUPPORTED_MARGIN_LIQUIDATION_MODEL" for d in r.warnings)
