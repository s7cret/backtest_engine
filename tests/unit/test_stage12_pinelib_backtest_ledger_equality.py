...
pinelib = pytest.importorskip(
    "pinelib", reason="pinelib is an optional legacy integration test dependency"
)
PineBar = pinelib.Bar
PineRuntime = pinelib.PineRuntime
RuntimeConfig = pinelib.RuntimeConfig
StrategyContext = pinelib.StrategyContext
SymbolInfo = pinelib.SymbolInfo
TimeframeInfo = pinelib.TimeframeInfo
...
def pine_rt(strategy: StrategyContext) -> PineRuntime:
    rt = PineRuntime(
        SymbolInfo("S", mintick=0.01),
        TimeframeInfo.from_string("D"),
...
def pine_bar(b: Bar) -> PineBar:
    return PineBar(
        b.time, b.open, b.high, b.low, b.close, volume=0.0, time_close=b.time_close
    )
...
def run_pine_market_entry_close_all(
...
def test_pinelib_backtest_process_orders_on_close_ledger_equality():
    be = BacktestEngine(be_cfg(process_orders_on_close=True)).run(
        MarketEntryCloseAll, bars=BARS
    )
...
def test_pinelib_backtest_commission_slippage_supported_subset_equality():
    ...
    pl = run_pine_market_entry_close_all(
        commission_type="percent", commission_value=1, slippage=0.5
    )
