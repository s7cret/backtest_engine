...
class BuyThenTrailing:
    ...
    def _process_bar(self, bar, bar_index):
        ...
        if bar_index==2: self.ctx.exit('XT',from_entry='L',qty=1,trail_points=2,trail_offset=1)


def test_trailing_stop_creates_executable_exit():
    bars=[Bar(1,10,10,10,10),Bar(2,10,10,10,10),Bar(3,10,10,10,10),Bar(4,11,13,11,12),Bar(5,12,12,10,10)]
...
class BuyThenProfitLoss:
    ...
    def _process_bar(self, bar, bar_index):
        if bar_index==0: self.ctx.entry('L','long',qty=1)
        if bar_index==2: self.ctx.exit('XP',from_entry='L',qty=1,profit=3,loss=2)


def test_profit_loss_convert_to_limit_stop_exits():
    bars=[Bar(1,10,10,10,10),Bar(2,10,10,10,10),Bar(3,10,10,10,10),Bar(4,10,13,9,12)]
...
def test_from_entry_matching_and_oca_reservation_do_not_fifo_close_wrong_entry():
    ...
    assert not any(d.order_id=='X1' and d.code=='ORDER_REJECTED_NO_AVAILABLE_POSITION_QTY' for d in r.warnings)
...
class RuntimeProbe:
    def __init__(self): self.begin=0; self.end=0
    def begin_bar(self, bar, bar_index): self.begin+=1
    def end_bar(self): self.end+=1
...
def test_early_stop_still_calls_end_bar_and_callback():
    ...
    r=BacktestEngine(cfg(runtime=runtime,early_stop_enabled=True,min_equity_stop=9995)).run(BuyAndDrop,bars=bars,callbacks=cb)
...
def test_batch_thread_backend_and_required_metrics():
    bars=[Bar(1,10,10,10,10),Bar(2,10,11,10,11),Bar(3,11,11,9,9),Bar(4,9,12,9,12)]
    c=cfg(required_metrics={'sharpe','sortino'})
    out=BatchBacktestRunner(c,backend='thread',max_workers=2).run([BacktestJob('a',BuyAndDrop,bars=bars),BacktestJob('b',BuyAndDrop,bars=bars)])
    assert set(out)=={'a','b'}
...
def test_cli_benchmark_and_batch(tmp_path: Path):
    strat=tmp_path/'strat.py'; bars=tmp_path/'bars.json'; jobs=tmp_path/'jobs.json'
    ...
    bench_out=tmp_path/'bench.json'; batch_out=tmp_path/'batch.json'
    assert cli_main(['benchmark','--strategy',str(strat),'--class','S','--bars',str(bars),'--symbol','S','--timeframe','1D','--output',str(bench_out),'--runs','1'])==0
