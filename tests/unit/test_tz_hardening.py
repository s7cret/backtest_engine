...
from backtest_engine import BacktestConfig, BacktestEngine, Bar, BarSeries, BacktestCallbacks
...
class BuyAtVisibleIndex:
    def __init__(self, params, runtime, ctx): self.ctx=ctx; self.seen=[]
    def _process_bar(self, bar, bar_index):
        self.seen.append(bar.time)
        if bar_index == 0: self.ctx.entry('L','long',qty=1)

class BuyThenReadState:
    def __init__(self, params, runtime, ctx): self.ctx=ctx; self.reads=[]
    def _process_bar(self, bar, bar_index):
        if bar_index == 0: self.ctx.entry('L','long',qty=1)
        if bar_index == 2:
...
class Provider:
    def __init__(self): self.called=0; self.lower_called=0
    def get_bars(self,*a):
        self.called+=1; return BARS
...
class LimitEntry:
    ...
    def _process_bar(self, bar, bar_index):
        if bar_index == 0: self.ctx.entry('L','long',qty=1,limit=10)

def test_run_slices_to_config_range_and_honors_max_bars_back():
    r=BacktestEngine(cfg(start_time=2,end_time=3,max_bars_back=1)).run(BuyAtVisibleIndex,bars=BARS)
...
def test_callback_disable_policy_records_once_and_continues():
    calls={'n':0}
    def bad(*args):
        calls['n']+=1; raise RuntimeError('boom')
    cbs=BacktestCallbacks(on_bar_start=bad)
    r=BacktestEngine(cfg(callback_error_policy='disable_callbacks')).run(BuyAtVisibleIndex,bars=BARS,callbacks=cbs)
    assert calls['n']==1
...
def test_config_validation_margin_and_streaming_compare():
    with pytest.raises(ConfigError):
        BacktestEngine(cfg(margin_long=50,unsupported_margin_policy='error')).run(BuyAtVisibleIndex,bars=BARS)
    with pytest.raises(ConfigError):
        BacktestEngine(cfg(tradingview_compare_mode='streaming',execution_mode='normal')).run(BuyAtVisibleIndex,bars=BARS)


def test_bar_magnifier_uses_provider_lower_timeframe():
    p=Provider()
    r=BacktestEngine(cfg(data_provider=p,use_bar_magnifier=True,bar_magnifier_lower_tf='60')).run(LimitEntry)
    assert p.called == 1
