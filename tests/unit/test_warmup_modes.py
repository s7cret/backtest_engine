...
def test_reset_broker_does_not_flush_warmup_entry_on_first_score_bar() -> None:
    result = BacktestEngine(_cfg(warmup_mode="CALC_THEN_RESET_BROKER")).run(
        BuyOnLastWarmupBar, bars=BarSeries.from_bars(BARS), effective_pre_bars=2
    )
    assert not result.closed_trades
    assert not result.open_trades
