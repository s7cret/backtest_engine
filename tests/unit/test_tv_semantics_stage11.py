...
def cfg(**kw):
    d = dict(symbol="S", timeframe="1D", start_time=1, end_time=5, commission_type="none", initial_capital=1000)
    d.update(kw)
...
def test_long_marketable_limit_exit_fills_next_open_not_literal_limit():
    bars = [Bar(1, 10, 10, 10, 10), Bar(2, 11, 12, 9, 11), Bar(3, 12, 13, 10, 12), Bar(4, 15, 16, 14, 15)]
...
def test_long_take_profit_limit_touched_intrabar_fills_at_limit():
    bars = [Bar(1, 10, 10, 10, 10), Bar(2, 11, 12, 9, 11), Bar(3, 12, 12, 10, 12), Bar(4, 12, 14, 10, 12)]
...
def test_non_marketable_limit_remains_pending_until_touched():
    bars = [Bar(1, 10, 10, 10, 10), Bar(2, 11, 12, 9, 11), Bar(3, 12, 13, 10, 12), Bar(4, 14, 19, 13, 14), Bar(5, 14, 21, 13, 14)]
...
def test_intrabar_drawdown_uses_open_trade_adverse_price():
    bars = [Bar(1, 10, 10, 10, 10), Bar(2, 10, 11, 5, 10), Bar(3, 10, 10, 10, 10), Bar(4, 10, 10, 10, 10)]
    r = BacktestEngine(cfg(end_time=4)).run(LongMarketableLimitExit, bars=bars)
