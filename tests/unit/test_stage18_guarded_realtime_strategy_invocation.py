...
from backtest_engine.core.realtime import BarTickSlice, RealtimeTickCommitPolicy, RuntimeTickUpdate
...
class RuntimeWithRealtimeTicks:
    ...
    def update_realtime_tick(self, tick: RuntimeTickUpdate) -> Bar:
        ...
        self.value += 1
        return Bar(time=tick.time or 0, open=10, high=tick.price, low=10, close=tick.price, volume=tick.volume)
...
def test_guarded_strategy_tick_invocation_rolls_back_runtime_strategy_and_broker() -> None:
    engine = _engine()
...
def test_guarded_strategy_tick_invocation_without_runtime_update_uses_parent_bar() -> None:
    engine = _engine()
    ctx = StrategyContext(engine.config, engine.state)
    strategy = StrategyRecordsTickBars()

    engine._guarded_realtime_strategy_tick_loop_skeleton(_slice(), ctx=ctx, strategy=strategy)

    assert strategy.seen_closes == []
    assert strategy.flag == "before"


def test_guarded_strategy_tick_invocation_can_commit_only_final_tick_in_skeleton() -> None:
    engine = _engine()
    ctx = StrategyContext(engine.config, engine.state)
    runtime = RuntimeWithRealtimeTicks()
    strategy = StrategyRecordsTickBars()
