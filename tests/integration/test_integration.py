...
class Bracket:
    ...
    def _process_bar(self,bar,bar_index):
        ...
        if bar_index==2: self.ctx.exit('X',limit=12,stop=8)
class Rev:
    def __init__(self,params,runtime,ctx): self.ctx=ctx
    def _process_bar(self,bar,bar_index):
        if bar_index==0: self.ctx.entry('L','long',qty=1)
        if bar_index==1: self.ctx.entry('S','short',qty=1)

def cfg(): return BacktestConfig(symbol='S',timeframe='1D',start_time=1,end_time=4,commission_type='none')
def test_bracket_and_reversal_and_batch():
    ...
    out=BatchBacktestRunner(cfg()).run([BacktestJob('a',Bracket,bars=B),BacktestJob('b',Rev,bars=B)])
    assert set(out)=={'a','b'}
