...
def make_bars(
    n: int, start_ms: int = 1700000000000, step_ms: int = 900_000
) -> BarSeries:
...
def test_prehistory_position_carries_into_score_window():
    ...
    assert (
        result.open_trades[0].entry_bar_index == 1
    )  # filled bar (order placed at bar 0)
...
def test_phase_trades_entry_prehistory_exit_score():
    ...
    engine = BacktestEngine(config)
    result = engine.run(
        PrehistoryEntryScoreCloseStrategy,
        bars=bars,
        effective_pre_bars=5,
    )
...
def test_prehistory_only_trade_excluded_from_score():
    ...
    engine = BacktestEngine(config)
    result = engine.run(
        PrehistoryEntryPrehistoryCloseStrategy,
        bars=bars,
        effective_pre_bars=5,
    )
    ...
    assert (
        len(score_trades) == 0
    ), "Prehistory-only trade should not appear as score trade"
    # D5-E: score_net_profit is the score-window metric; net_profit is full-window
    assert (
        result.score_net_profit == 0.0
...
def test_bars_processed_is_score_window_count():
    ...
    assert (
        result.bars_processed >= 8
    ), f"Expected >= 8 score bars, got {result.bars_processed}"
