...
def _load_bars(path: str):
    ...
    if isinstance(rows, dict) and "bars" in rows:
        rows = rows["bars"]
...
def main(argv=None) -> int:
    ...
    run.add_argument("--class", dest="cls", required=True)
    ...
    run.add_argument("--end", type=int, default=2**63 - 1)
    run.add_argument("--capital", type=float, default=10000)
    run.add_argument("--params", default=None)
    run.add_argument("--execution-mode", default="normal")
    run.add_argument("--output", required=True)
    run.add_argument("--no-events", action="store_true")
    run.add_argument("--no-equity-curve", action="store_true")
    cmp = sub.add_parser("compare")
    cmp.add_argument("--our", required=True)
    cmp.add_argument("--tv", required=True)
    cmp.add_argument("--output", required=True)
    cmp.add_argument("--price-tolerance", type=float, default=0.0)
    cmp.add_argument("--qty-tolerance", type=float, default=0.0)
    cmp.add_argument("--format", choices=["json", "text"], default="json")
    exp = sub.add_parser("export")
    exp.add_argument("--input", required=True)
    exp.add_argument("--trades-csv")
    exp.add_argument("--summary-md")
    bench = sub.add_parser("benchmark")
    bench.add_argument("--strategy", required=True)
    bench.add_argument("--class", dest="cls", required=True)
    bench.add_argument("--bars", required=True)
    bench.add_argument("--symbol", required=True)
    bench.add_argument("--timeframe", required=True)
    bench.add_argument("--start", type=int, default=0)
    bench.add_argument("--end", type=int, default=2**63 - 1)
    bench.add_argument("--capital", type=float, default=10000)
    bench.add_argument("--params")
    bench.add_argument("--runs", type=int, default=3)
    bench.add_argument("--output", required=True)
    bench.add_argument("--format", choices=["json", "text"], default="json")
    bat = sub.add_parser("batch")
    bat.add_argument("--strategy", required=True)
    ...
    if a.cmd == "run":
        ...
        cfg = BacktestConfig(
            ...
            collect_events=not a.no_events,
            collect_equity_curve=not a.no_equity_curve,
        )
        res = BacktestEngine(cfg).run(
            _load_class(a.strategy, a.cls), params=params, bars=_load_bars(a.bars)
        )
        JSONResultWriter().write(res, a.output)
        return 0
    if a.cmd == "compare":
    ...
    if a.cmd == "batch":
        ...
        jobs = [
            BacktestJob(
                ...
                strategy,
                params=j.get("params", {}),
                config_overrides=j.get("config_overrides", {}),
                bars=bars,
            )
            for j in specs
        ]
    ...
    if a.cmd == "export":
        ...
        if a.summary_md:
            lines = [
                "# Backtest summary",
                f"final_equity: {result.get('final_equity')}",
                f"net_profit: {result.get('net_profit')}",
                f"total_trades: {result.get('total_trades')}",
            ]
            Path(a.summary_md).write_text("\n".join(lines) + "\n")
        return 0
    print("Commands: run, compare, export, benchmark, batch.")
    return 0
