...
def compare_trades(our_trades: Iterable[Any], reference_trades: Iterable[Any], *, price_tolerance: float = 0.0, qty_tolerance: float = 0.0) -> ComparisonReport:
    ...
    if len(ours) != len(refs):
        first = n
        diags.append(Diagnostic('TRADINGVIEW_COMPARE_MISMATCH', 'trade count mismatch', 'warning', context={'our_count': len(ours), 'reference_count': len(refs)}))
    for i in range(n):
        ...
        for f in fields:
            ...
            if mismatch:
                bad = True; ctx[f] = {'actual': a, 'expected': b, 'tolerance': tol}
        if bad:
            if first is None: first = i
            diags.append(Diagnostic('TRADINGVIEW_COMPARE_MISMATCH', 'trade field mismatch', 'warning', context=ctx))
            break
    return ComparisonReport(matched=not diags, our_count=len(ours), reference_count=len(refs), first_mismatch_index=first, summary={'checked': n}, diagnostics=diags)
