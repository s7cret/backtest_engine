...
class EarlyStopChecker:
    ...
    def check(
        ...
        *,
        ...
        if self.config.max_drawdown_stop_percent is not None and drawdown_percent >= self.config.max_drawdown_stop_percent:
            return EarlyStopDecision(True, 'max_drawdown_stop_percent')
        if self.config.max_bars_without_trade is not None and last_trade_bar is not None and bar_index - last_trade_bar >= self.config.max_bars_without_trade:
